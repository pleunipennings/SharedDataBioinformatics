...
getWTAA<-function(DF){ #we make a fundtion that takes a data frame as input
    ...
    for(x in seq(1, length(cons) - 2, 3)){ # x is the looping variable, 
        #it goes from 1 until 2 less than the length of cons, and in goes in steps of three, 
        #so its value will be 1, 4, 7, 10 etc. 
        codon <- c(cons[x], cons[x+1], cons[x+2]) #get the codon that "belongs" to x, so for example, when x is 1, the codon is the nucleotides at 1, 2, 3
    ...
    if (length(which(names(DF)=="WTAA"))==0){ #check whether the dataframe has a WTAA column
